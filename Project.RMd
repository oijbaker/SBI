...
sourceCpp(code = "
  ...
  // [[Rcpp::export]]
  NumericVector SIR(NumericVector s, NumericVector i, NumericVector r, double s0,
                    double i0, double r0, double beta, double gamma) {
    s[0] = s0;
    i[0] = i0;
    r[0] = r0;
...
ggplot(aes(x=day,y=sir), data = sir)+
  ...
  labs(y="Proportion of Infected",x="Day",title="SIR Model with beta=0.8, gamma=0.2")
        +theme_light()
```
...
double mean_std_dist(double* x_sim, NumericVector x, NumericVector w) {
  ...
  double total = w[0]*pow(mean-13.14,2)/pow(13.14,2)+w[1]*pow(sd-2.19,2)/pow(2.19,2)
                 +w[2]*pow(max_double(x_sim)-max(x),2)/(pow(max(x),2));
...
// [[Rcpp::export]]
NumericMatrix ABC(int n, double eps, int p, NumericVector x,
                  int ncores, int metric, NumericVector w)
{
...
before_beta_plot<-ggplot(aes(x=accepted_samples_before[,1]),
  ...
  labs(x="Beta",y="Frequency",
       title=expression(paste("Distribution of ", beta,"  (Before)")))+
  theme_light()
...
before_gamma_plot<-ggplot(aes(x=accepted_samples_before[,2]),
  ...
  geom_density(fill="pink",color="red")+
  xlim(c(0,1))+
  labs(x="Gamma",y="Density",title=expression(paste("Distribution of ", gamma," (Before)")))+
  theme_light()
...
after_beta_plot<-ggplot(aes(x=accepted_samples_after[,1]),
  ...
  geom_density(fill="lightblue",color="blue")+
  xlim(c(0,3))+
  labs(x="Beta",y="Frequency",title=expression(paste("Distribution of ", beta, " (After)")))+
  theme_light()
...
after_gamma_plot<-ggplot(aes(x=accepted_samples_after[,2]),
  ...
  geom_density(fill="pink",color="red")+
  xlim(c(0,1))+
  labs(x="Gamma",y="Density",title=expression(paste("Distribution of ", gamma, " (After)")))+
  theme_light()
...
ggplot(aes(x=datas[[1]]),data=as.data.frame(datas[[1]]))+
  ...
  xlim(c(0,3))+
  labs(x="Beta",y="Frequency",
       title=expression(paste("Distribution of ", beta,"  as  ", epsilon, " approaches 0.1")))+
  theme_light()
...
Optimal_Plot<-function(sims,plot.heat){
  ...
  fit_plot <- ggplot(data=data.frame(z=c(seq(1,20),seq(1,20)),y=c(obs_data,I),
    Data=c(rep("Observed",20  ),rep("Simulation",20))),aes(x=z,y=y,col=Data)) +
    geom_point() + geom_line() + xlab("Day") + ylab("Proportion of Infected") +
